...
export class ReminderEmailService {
  constructor() {
    ...
    this.defaultFacultyList = [
      { id: 'fac-1', name: 'Dr. Rajesh Jambhulkar', email: 'rajesh.j@pwmeded.edu.in', phone: '98234 56710', dept: 'Biochemistry', role: 'HOD & Professor • Biochemistry', status: 'Verified', cohorts: ["Prarambh '26", "Sushruta '26"] },
      { id: 'fac-2', name: 'Dr. Pradeep Pawar', email: 'pradeep.p@pwmeded.edu.in', phone: '98450 12389', dept: 'Anatomy', role: 'Professor • Anatomy', status: 'Verified', cohorts: ["Prarambh '26"] },
      { id: 'fac-3', name: 'Dr. Vivek Nalgirkar', email: 'vivek.physio@pwmeded.edu.in', phone: '99881 23411', dept: 'Physiology', role: 'Professor • Physiology', status: 'Verified', cohorts: ["Sushruta '26"] },
  ...
  getFacultyOnboardingList() {
    try {
      if (typeof localStorage === 'undefined') return this.defaultFacultyList;
      const data = localStorage.getItem(this.ONBOARDING_KEY);
      return data ? JSON.parse(data) : this.defaultFacultyList;
    } catch (e) {
      return this.defaultFacultyList;
    }
  }
  ...
  saveFacultyOnboardingList(list) {
    try {
      if (typeof localStorage !== 'undefined') {
        localStorage.setItem(this.ONBOARDING_KEY, JSON.stringify(list));
      }
      this.broadcastEvent('meded:faculty_onboarding_updated', list);
    } catch (e) {
      console.error('Error saving onboarding list:', e);
    }
  }
